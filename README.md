...
My journal is rendered as one central marbled pothos. Every category is a curved main vine from that plant; saved parts are olive offshoot leaves; helpful outcomes are healthy green leaves; and painful outcomes are dry rust leaves. No other plant species are used in the map. The botanical SVG uses dainty asymmetric leaf silhouettes, layered gradients, natural veins, irregular cream marbling, restrained shadows, dry-leaf mottling, and a small shaded ceramic pot. Main vines and descendant offshoots use fine, irregular cubic curves; visual leaves stay delicate while separate transparent hit areas remain large enough for phone use. Existing `state.selfMap` records feed the renderer without migration or data loss.
